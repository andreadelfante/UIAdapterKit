//
//  ArraySearchableTableViewAdapter.swift
//  UIAdapterKit
//
...
open class ArraySearchableTableViewAdapter: ArrayTableViewAdapter {
    ...
    private var filteredSections: [BaseTableViewSection]
    ...
    open override func append<T>(section: ArrayTableViewSection<T>) {
    ...
    @discardableResult
    open override func insert<T>(section: ArrayTableViewSection<T>, at index: Int) -> Bool {
    ...
    @discardableResult
    open override func replace<T>(section: ArrayTableViewSection<T>, at index: Int) -> Bool {
        ...
        if payload != nil {
            ...
            DispatchQueue(label: "\(String(describing: self)).FilterQueue").async { [weak self] in
                ...
                for var section in self.sections {
                    if let filterableSection = section.copy() as? (BaseTableViewSection & PerformableFilter) {
