//
//  ArrayTableViewAdapterTests.swift
//  UIAdapterKit_Tests
//
...
class ArrayTableViewAdapterTests: XCTestCase {
    private var section: ArrayTableViewSection<BasicModel>!
    private var anotherSection: ArrayTableViewSection<BasicModel>!
    private var adapter: ArrayTableViewAdapter!
    
    override func setUp() {
        super.setUp()
        
        section = ArrayTableViewSection(items: BasicModel.fake(3), itemBuilder: { BasicTableViewItem($0) })
        anotherSection = ArrayTableViewSection(items: BasicModel.fake(2), itemBuilder: { BasicTableViewItem($0) })
        adapter = ArrayTableViewAdapter()
    }
